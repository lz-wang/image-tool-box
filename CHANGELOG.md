...
- **收尾 5** `fix(core)` 收紧源校验与 domain 契约：`filehash.SumFile` 拒绝非普通文件（FIFO 在 open 阶段阻塞前即以 `E_FILE_READ` 拒绝，目录同理）；inspect 对识别为 SVG 的文件执行完整 XML 结构校验（解析到 EOF、单一根、根后无内容），截断/损坏的 SVG 默认输出 `error.structure_invalid` 且 `--strict` 失败（此前静默通过），`--detail.detected_by` 收敛为真实检测来源（`magic` / `svg.parse` / `image.DecodeConfig`）；S3 domain 补齐 Normalize/Validate（`Config` 拒绝负数 max-attempts/超时、`ListOptions` 拒绝超范围 page-size 与负 limit、`DownloadOptions`/`UploadOptions` 承接 digest/expect-size/if-exists/skip 互斥校验）——domain 是参数规则唯一事实来源，CLI validator 保留为快速 UX 前置。
